...
)
{
	...
	if(g_onoff == G_WLAN_SET_ON) {
		if((error = dev_wlc_bufvar_get(dev, "counters", (char *)&cnt, sizeof(wl_cnt_t))) >= 0) {
			p += snprintf(p, MAX_WX_STRING, "dwBytesSent=%u ", dtoh32(cnt.txbyte));
			p += snprintf(p, MAX_WX_STRING, "dwBytesRcvd=%u ", dtoh32(cnt.rxbyte));
			p += snprintf(p, MAX_WX_STRING, "dwPktDataSent=%u ", dtoh32(cnt.txframe));
			p += snprintf(p, MAX_WX_STRING, "dwPktDataRcvd=%u ", dtoh32(cnt.rxframe));
			p += snprintf(p, MAX_WX_STRING, "dwPktMgmtSent=%u ", dtoh32(cnt.txfrmsnt));
			p += snprintf(p, MAX_WX_STRING, "dwPktNonRCtsSent=%u ", dtoh32(cnt.txnocts));
			p += snprintf(p, MAX_WX_STRING, "dwPktRCtsSent=%u ", dtoh32(cnt.txrtsfrm)+dtoh32(cnt.txctsfrm));
			p += snprintf(p, MAX_WX_STRING, "dwPktCrcErrRcvd=%u ", dtoh32(cnt.rxcrc));

			wrqu->data.length = p - extra + 1;
		}
		else
			WL_ERROR(("%s: get counter error\n", __FUNCTION__));
	}
...
)
{
	...
	if (g_onoff == G_WLAN_SET_ON) {
     ...
     netif_carrier_off(dev); //2012-08-23, PS4_WIFI, fix for DNS query issue
#if defined(WL_IW_USE_ISCAN)
	...
	struct iw_request_info *info, wl_bss_info_t *bi)
{
	...
	if (bi->ie_length) {
		...
		ptr = ((uint8 *)bi) + sizeof(wl_bss_info_t);
		ptr_len = bi->ie_length;
		if ((ie = bcm_parse_tlvs(ptr, ptr_len, DOT11_MNG_COUNTRY_ID))) {
			iwe.cmd = IWEVCUSTOM;
			iwe.u.data.length = strlen("country_ie=") + 2*(ie->len + 2);
			memcpy(buf, "country_ie=", 11);
			wpa_snprintf_hex(buf + 11, 2+1, &(ie->id), 1);
			wpa_snprintf_hex(buf + 13, 2+1, &(ie->len), 1);
			wpa_snprintf_hex(buf + 15, 2*ie->len+1, ie->data, ie->len);
			event = IWE_STREAM_ADD_POINT(info, event, end, &iwe, buf);
		}

		ptr = ((uint8 *)bi) + sizeof(wl_bss_info_t);
		ptr_len = bi->ie_length;
		if ((ie = bcm_parse_tlvs(ptr, ptr_len, DOT11_MNG_HT_CAP))) {
			iwe.cmd = IWEVCUSTOM;
			iwe.u.data.length = strlen("htcap_ie=") + 2*(ie->len + 2);
			memcpy(buf, "htcap_ie=", 9);
			wpa_snprintf_hex(buf + 9, 2+1, &(ie->id), 1);
			wpa_snprintf_hex(buf + 11, 2+1, &(ie->len), 1);
			wpa_snprintf_hex(buf + 13, 2*ie->len+1, ie->data, ie->len);
			event = IWE_STREAM_ADD_POINT(info, event, end, &iwe, buf);
	...
	char *ext)
{
   ...
   net_os_set_suspend_disable(dev, 0); //2012-01-18, PS4_WIFI, Kyeongyeon Song, PLM2392 , Exeltech
	WL_SOFTAP(("%s Done with %d\n", __FUNCTION__, res));
	DHD_OS_MUTEX_UNLOCK(&wl_softap_lock);
	net_os_wake_unlock(dev);

	//return res;
	return 0; //20111120_SKY
}
...
#endif 
  net_os_set_suspend_disable(dev, 1); //2012-01-18, PS4_WIFI, Kyeongyeon Song, PLM2392 , Exeltech
...
/* SoftAP Cert Only */
#ifdef AP_CERT_FOR_BG
	{
		int gp_override = 0;
		dev_wlc_ioctl(dev, WLC_SET_GMODE_PROTECTION_OVERRIDE, &gp_override, sizeof(gp_override));
	}
#endif
   printk("%s\n", "Set default dtim to 1");
	/* Set default dtim to 1 */
	{
		int dtim = 1;
...
)
{
	...
	if (dwrq->length && extra) {
	    ...
	    if (strnicmp(extra, "SCAN-ACTIVE", strlen("SCAN-ACTIVE")) == 0) {
			...
			ret = wl_iw_get_counters(dev, info, (union iwreq_data *)dwrq, extra);
#endif
#ifdef SOFTAP
...
#	define WL_IW_DONT_CARE	9999
	const conn_fail_event_map_t event_map [] = {
		...
		"Conn", "RsnMismatch"},
#ifdef ATT_CIQ_SUPPORT
		{WLC_E_AUTH,         WL_IW_DONT_CARE,        WL_IW_DONT_CARE,
		"Conn", "Auth"},
#else
...
#endif
#ifdef ATT_CIQ_SUPPORT
		{WLC_E_ASSOC,        WL_IW_DONT_CARE,        WL_IW_DONT_CARE,
		"Conn", "Assoc"},
		{WLC_E_REASSOC,      WL_IW_DONT_CARE,        WL_IW_DONT_CARE,
		"Conn", "Reassoc"},
#else
...
wl_iw_event(struct net_device *dev, wl_event_msg_t *e, void* data)
{
...
#ifdef ATT_SOFTAP_EVENT
        else
        {
             char mac_buf[32] = {0};
             WL_SOFTAP(("STA_BLOCK %02X:%02X:%02X:%02X:%02X:%02X",
                             macaddr[0], macaddr[1], macaddr[2],
                             macaddr[3], macaddr[4], macaddr[5]));
	...
	case WLC_E_DISASSOC_IND:
		...
		bzero(&extra, ETHER_ADDR_LEN);
		break;
#ifdef CUSTOMER_HW_PT //20111120_SKY
  case WLC_E_DEAUTH:
...
#endif 
			WL_TRACE(("Link UP\n"));
        netif_carrier_on(dev); //2012-08-23, PS4_WIFI, fix for DNS query issue
		}
		net_os_wake_lock_timeout_enable(dev);
		wrqu.addr.sa_family = ARPHRD_ETHER;
		break;
