...
//#define FEATURE_USB_MANAGER_SYNC_BUG_WORKAROUND //pz1945 :TODO need to analyze
...
static bool b_factory_mode_disabled;
...
/*defined(CONFIG_PANTECH_DOMESTIC) */
	{
		...
		{"ptp",
			PTP_TYPE_FLAG,
			0x10A9, 0x1105, "Pantech Config 22",
		...
		{"mtp,serial,diag,obex", 
			ACM_TYPE_FLAG | DIAG_TYPE_FLAG | MTP_TYPE_FLAG | OBEX_TYPE_FLAG , 
			0x10A9, 0x1104, 
			"Pantech Config 24", 
			USB_CLASS_COMM, 0x00, 0x00},

		{"mtp,serial,diag,obex,adb", 
			ACM_TYPE_FLAG | DIAG_TYPE_FLAG | ADB_TYPE_FLAG | MTP_TYPE_FLAG | OBEX_TYPE_FLAG , 
			0x10A9, 0x1104, 
			"Pantech Config 24", 
			USB_CLASS_COMM, 0x00, 0x00},

		{"ptp,serial,diag,obex", 
			ACM_TYPE_FLAG | DIAG_TYPE_FLAG | PTP_TYPE_FLAG | OBEX_TYPE_FLAG , 
			0x10A9, 0x1104, 
			"Pantech Config 25", 
			USB_CLASS_COMM, 0x00, 0x00},

		{"ptp,serial,diag,obex,adb", 
			ACM_TYPE_FLAG | DIAG_TYPE_FLAG | ADB_TYPE_FLAG | PTP_TYPE_FLAG | OBEX_TYPE_FLAG , 
			0x10A9, 0x1104, 
			"Pantech Config 25", 
			USB_CLASS_COMM, 0x00, 0x00},
	...
	/*defined(CONFIG_PANTECH_VERIZON)*/
	{
		...
		{"ptp",
			PTP_TYPE_FLAG,
			0x10A9, 0x6035, "Pantech Config 22",
			USB_CLASS_STILL_IMAGE, 1, 1},
	...
	/* defined(CONFIG_PANTECH_ATNT)*/
	{
		...
		{"mtp", 
			MTP_TYPE_FLAG, 
			0x10A9, 0x6050, "Pantech Config 5",
			USB_CLASS_VENDOR_SPEC, USB_SUBCLASS_VENDOR_SPEC, 0},

		{"ptp",
			PTP_TYPE_FLAG,
			0x10A9, 0x6050, "Pantech Config 22",
			USB_CLASS_STILL_IMAGE, 1, 1},
		...
		{"serial,diag,rmnet_sdio",
			ACM_TYPE_FLAG | DIAG_TYPE_FLAG | RMNET_TYPE_FLAG,
			0x10A9, 0x6050,
			"Pantech Config 15",
			USB_CLASS_COMM,0x00, 0x00},

		{"serial,diag,adb,rmnet_sdio",
			ACM_TYPE_FLAG | DIAG_TYPE_FLAG | ADB_TYPE_FLAG | RMNET_TYPE_FLAG,
			0x10A9, 0x6050,
			"Pantech Config 16",
			USB_CLASS_COMM, 0x00, 0x00},
	...
	/* defined(JAPAN) */
	{
		...
		{"ptp",
			PTP_TYPE_FLAG,
			0x10A9, 0x6059, "Pantech Config 22",
			USB_CLASS_STILL_IMAGE, 1, 1},
...
void usb_connect_cb(void){
	...
	printk("pz1945 - usb_connect_cb usb_manager_state_enabled: %d \n", usb_manager_state_enabled);
...
void mode_switch_cb(int mode)
{
	struct device_mode_change_dev *dev_mode_change =
	    _device_mode_change_dev;
	int ret;
	printk("pz1945 - mode_switch_cb  mode: %x \n",mode );
...
int type_switch_cb(int type)
{
	struct device_mode_change_dev *dev_mode_change =
	    _device_mode_change_dev;
	int ret;
#ifdef FEATURE_USB_MANAGER_SYNC_BUG_WORKAROUND //pz1945 :TODO need to analyze	
	static int skip=1;
#endif
	...
	printk("B get_func_thru_type  \n");
	ret = get_func_thru_type(type);
	printk("E get_func_thru_type  \n");
...
#endif	
	dev_mode_change->pc_mode_switch_flag = ret + 1;
	printk("B  wake_up_interruptible  \n");
...
static void android_enable_function_cb(char *functions)
{
	...
	while(functions){
		...
		if(func){
			...
			}else if(!strcmp(func, "rmnet_sdio")){
	...
	if(type){
#ifdef CONFIG_ANDROID_PANTECH_USB_FACTORY_CABLE
		if(!b_factory_mode_disabled && b_factory_mode_request){
...
static ssize_t factory_mode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	sscanf(buf, "%d", &value);
	if(value){
		b_factory_mode_disabled = false;
	}else{
		b_factory_mode_disabled = true;
	}
	return size;
}
...
static int device_mode_change_release(struct inode *ip, struct file *fp)
{
	...
	printk(KERN_ERR "tarial %s : mode change release\n", __func__);
			 ...
			 size_t count, loff_t *ppos)
{
	...
	printk(" device_mode_change_write \n");
	...
	printk(KERN_ERR "%s Mode change command=[%s]\n", __func__, cmd);
	...
	if(strncmp(cmd, "carrier_mode", 12) == 0){
		...
		printk(KERN_ERR "%s - carrier_mode [%d]\n", __func__,pantech_usb_carrier);
		return count;
	}
	...
	if (strncmp(cmd, "usb_disconnect", 14) == 0) {
		printk(KERN_ERR "%s - Handled disconnect[%d]\n", __func__, _android_dev->enabled);
		down_write(&semaphore);
		if(_android_dev->enabled){
			usb_gadget_disconnect(_android_dev->cdev->gadget);
			usb_remove_config(_android_dev->cdev, &android_config_driver);
	...
	printk(KERN_ERR "%s - Successfully enabled function - %s \n",
		__func__, cmd);
...
int set_factory_mode(bool onoff)
{
  ...
  if(onoff == true){
      ...
      usb_reconnect_cb(DEFAULT_TYPE_FLAG);
  }
