...
/* 
   2011.12.15. WSH, MIPI_CSI configration전에 camera sensor로 부터 frame이 들어올 경우 MIPI단에 문제가 생기는 경우발생. 
   이를 개선 하기 위해 MIPI_CSI configration수행 후에 camera init register를 set하게 수정함. 
*/
#define CONFIG_MACH_MSM8X60_MIPI_CSI_ERR_FIX
...
//	mdelay(1);
	...
	//msleep(500); /* 500ms PANTECH_CAMERA_TODO *///test cts
...
static int mt9v113_set_mode(int mode)
{
...
#ifdef CONFIG_MACH_MSM8X60_MIPI_CSI_ERR_FIX
       mdelay(100);//mdelay(300);  //wsh_test_3	

	rc = mt9v113_power_on();
	
	if (rc < 0)
		goto sensor_init_fail2;
	...
	case SENSOR_PREVIEW_MODE:
		if (sensor_mode != SENSOR_PREVIEW_MODE)
		{
			...
			/* 2011.11.18.WSH, MT9V113의 경우 snapshot/preview mode설정시 time delay를 주어야 fram붕괴가 안일어남. 반드시 time delay를 주어야 함. 
                          aptina에서는 snapshot/preview mode설정에 대해 사용하지 않을 것을 권고함. 
			*/
			mdelay(150);//mdelay(300);//test cts
		}
		break;
...
#ifndef CONFIG_MACH_MSM8X60_MIPI_CSI_ERR_FIX
	rc = mt9v113_power_on();
	if (rc < 0)
		goto sensor_init_fail;
