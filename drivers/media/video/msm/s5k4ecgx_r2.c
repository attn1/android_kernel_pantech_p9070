...
/*
   프레스토 1.8 IO전원의 경우 TP10에서 main/sub camera공용으로 사용하도록 회로 변경되어 
   main camera사용시 sub camera를 standby mode로 놓아야 한다.
   그렇지 않을 경우 시료 별로 임피던스 차이에 의해 main camera 초기화시 문제가 발생할수 있다. 
*/
#ifdef CONFIG_PANTECH_CAMERA_MT9V113
#define C_PANTECH_CAMERA_PRESTO_MT9V113_STANDBY
#endif
...
#ifdef C_PANTECH_CAMERA_PRESTO_MT9V113_STANDBY
//main camera 전원 - CAMV_IO_1P8V은 공용.
#define CAMV_IO_1P8V	0
#define CAMV_CORE_1P2V	1
#define CAMV_A_2P8V	2
#define CAMV_AF_2P8V	3
//sub camera 전원
#define CAMV_CORE_1P8V	4
#define CAMV_A_2P8V_L1	5
//#define CAMV_I2C_PU	6
#define CAMV_MAX	6

static svreg_ctrl_t svregs[CAMV_MAX] = {
	{CAMV_IO_1P8V,   "8901_mvs0", NULL, 1800},   //main/sub 공용 camera 전원
	{CAMV_CORE_1P2V, "8901_l0",   NULL, 1200},    //main camera 전원
	{CAMV_A_2P8V,    "8901_l2",   NULL, 2800},       //main camera 전원
	{CAMV_AF_2P8V,   "8901_l3",   NULL, 2800},      //main camera 전원
	{CAMV_CORE_1P8V, "8901_lvs2", NULL, 1800},  //sub camera 전원
	{CAMV_A_2P8V_L1,    "8901_l1",   NULL, 2800},	 //sub camera 전원
};
...
/* pz2428_120525 : 기존 AF시 무조건 발광에서 촬영전 AF시에 일시 발광후 촬영시 재발광하도록 수정함. */
#if defined(CONFIG_MACH_MSM8X60_PRESTO)
#define C_PANTECH_CAMERA_PRESTO_ICS_FLASH_AUTO
#endif
...
#ifdef C_PANTECH_CAMERA_PRESTO_MT9V113_STANDBY
	if (svreg_ctrl(svregs, CAMV_A_2P8V_L1, 1) < 0)	rc = -EIO;
	mdelay(1); /* > 0ms */
...
#ifdef C_PANTECH_CAMERA_PRESTO_MT9V113_STANDBY
       if (sgpio_ctrl(sgpios, CAMIO_F_STB, 1) < 0)	rc = -EIO;
#endif
...
//	(void)msm_camio_clk_disable(CAMIO_CAM_MCLK_CLK);
	...
	if (svreg_ctrl(svregs, CAMV_A_2P8V_L1, 0) < 0)	rc = -EIO;
#endif
...
#ifndef C_PANTECH_CAMERA_PRESTO_ICS_FLASH_AUTO
	if( autofocus == 0 )
#endif
	{
		if (flash_mode == 1) {
...
static int s5k4ecgx_auto_focus(void)
{
...
#ifdef C_PANTECH_CAMERA_PRESTO_ICS_FLASH_AUTO
	if( autofocus == 0 )
#endif
	{
		if (flash_mode == 1) {
	...
	if ((flash_mode == 1) || (flash_mode == 2))
        ...
        /* auto focus success 후 600msec이상을 줘야함. focusing을 한후에도 focusing 되지 않는 frame이 입력되는 경우를 방지하기 위해 충분한 시간 delay를 줘야함. */
	msleep(200);//msleep(600);

end: //auto focus end

#ifdef C_PANTECH_CAMERA_PRESTO_ICS_FLASH_AUTO
	if (flash_mode == 4)  /* flash mode가 1,2인경우 4로 set을 해주기 때문에 4로 체크한다. */
	{
		s5k4ecgx_set_flash(0);
		flash_mode = 2; /*off 이후 snapshot시 재발광을 위해서 다시 set해주도록 한다. */
	}
#endif
...
static int32_t s5k4ecgx_set_wb(int wb)
{
	...
	switch (wb) {
       /* 2011.11.03. WSH, camera유료 APP에서 white balance mode중 "OFF : 9"로 설정하는 메뉴가 있어 카메라 사진촬영 fail발생하는 현상 수정. OFF설정시 AUTO와 동일하게 설정한다. */		
       case 9:		
