...
#define SKY_SOC_EMPTY	9//pz1946 20110907 EF39S_EF40K_EF40S 15//pz19469 110826 default 9
//#define SKY_SOC_EMPTY	12
#define SKY_SOC_FULL	1159//pz1946 20110907 EF39S_EF40K_EF40S 15 1147//pz1946 110826 default 1179 
#elif defined(CONFIG_PANTECH_PRESTO_BOARD)	|| defined(CONFIG_PANTECH_QUANTINA_BOARD)//p14682 kobj add 110720
...
//#define SKY_SOC_EMPTY	32//56//36//56//8// 7 // chagne p14682 kobj 110816 7 ->8
...
//pz1946 20110915 EF39S EF40S EF40K common apply
	...
	if(ret<0)
	{
		...
		{
#if defined(CONFIG_MACH_MSM8X60_EF39S) || defined(CONFIG_MACH_MSM8X60_EF40K) ||defined(CONFIG_MACH_MSM8X60_EF40S)
                        ret=max17040_write_reg(MAX17040_RCOMP_MSB, 0xb0,0x00);
#elif defined(CONFIG_PANTECH_PRESTO_BOARD)			//p14682 kobj add 110720
                        ret=max17040_write_reg(MAX17040_RCOMP_MSB, 0xd7,0x00);
#else
			ret=max17040_write_reg(MAX17040_RCOMP_MSB, 0xc0,0x00);			
...
//pz1946 20110920 offline charging
	if(offline_charging_status) {
		init_count = 5; //20120319 PZ1949 form KBJ
	}
	else {
//[ 20120319 PZ1949 frim KBJ
...
#endif //] 20120319 PZ1949 frim KBJ
		initial_booting = 1;
		}
	}
	else {
...
//	int sky_state=0;
		...
		if(t_temp<60&&t_temp>0)
		{
			...
			//printk("max17040_get_soc f_rcomp %d \n",f_rcomp);		
		...
		//printk("max17040_get_soc rcomp app  \n");		
		ret=max17040_write_reg(MAX17040_RCOMP_MSB, f_rcomp,0x00);

		//printk("max17040_get_soc rcomp re app  \n");		
#if 1
		if(ret<0)
...
#else  //20120319 PZ1949 from KBJ  (condition added)
	//check i2c error
...
//pz1946 EF39S EF40K/EF40S empty level change
	...
	soc  = (soc * 104)/100;//20120319 PZ1949 from KBJ
#endif	
	//logic code
	if(soc>100) //soc>100
		soc=100;
